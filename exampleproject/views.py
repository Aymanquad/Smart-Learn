...
def Loginpage(request):
    ...
    return render(request, 'primprojects/SmartLearn-Login.html')

def Logoutuser(request):
    logout(request)
    return redirect('login')

def ppl_profile(request,pk):
    profile = profiles.objects.get(username=pk)
    return render(request, 'primprojects/profilepage.html', {"profile" : profile })
...
def registeruser(request):
    ...
    return render(request, 'primprojects/registration.html',context)
